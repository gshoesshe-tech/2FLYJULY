(() => {
  ...
  function updateConditions(){
    ...
    if(!isJnt){$('shippingFee').value='0';$('courierCost').value='';}
    updateTotals(true);
  }
  ...
  async function saveEditedOrder(){
    ...
    try{
      ...
      const order={raw_order_form:raw,form_fingerprint:current?.form_fingerprint||null,customer_name:TF.$('orderCustomer').value.trim(),phone:TF.$('orderPhone').value.trim(),address:TF.$('orderAddress').value.trim(),order_type:TF.$('orderType').value,fulfillment_method:TF.$('fulfillmentMethod').value,expected_release_date:TF.$('expectedReleaseDate').value||null,shipping_fee_due:TF.$('fulfillmentMethod').value==='jnt'?TF.num(TF.$('shippingFee').value):0,actual_courier_cost:TF.$('fulfillmentMethod').value==='jnt'&&TF.$('courierCost').value.trim()!==''?TF.num(TF.$('courierCost').value):0,notes:TF.$('orderNotes').value.trim()};
      ...
      const ship=await TF.state.supa.rpc('update_order_shipping',{p_order_id:editingOrderId,p_fulfillment_method:order.fulfillment_method,p_shipping_fee_due:order.shipping_fee_due,p_actual_courier_cost:TF.$('courierCost').value.trim()===''?null:TF.num(TF.$('courierCost').value)});if(ship.error)throw ship.error;
  ...
  async function openOrderDetails(order){
    ...
    TF.$('detailCourierCost').textContent=order.courier_cost_finalized?TF.money(order.actual_courier_cost):'Pending';
    TF.$('detailShippingProfit').textContent=order.courier_cost_finalized?TF.money(Number(order.shipping_fee_due||0)-Number(order.actual_courier_cost||0)):'Pending';
  ...
  async function saveDetailShipping(){
    ...
    try{
      ...
      const courier=method==='jnt'&&TF.$('detailCourierInput').value.trim()!==''?TF.num(TF.$('detailCourierInput').value):null;
  ...
  function openRefundDialog(){
    if(!activeDetailOrder)return;
    TF.$('refundDate').value=TF.today();
    TF.$('refundAmount').value=Number(activeDetailOrder.verified_total_paid||activeDetailOrder.total_due||0).toFixed(2);
    TF.$('refundType').value='full';
    TF.$('refundReturnStock').checked=['packing','shipped','delivered'].includes(activeDetailOrder.status);
    TF.$('refundAccount').innerHTML=TF.accountOptions(false);
    TF.$('refundReason').value='';
    TF.$('refundOrderMeta').textContent=`${activeDetailOrder.order_number} · ${activeDetailOrder.customer_name}`;
    TF.$('refundOrderDialog').showModal();
  }
  async function submitRefund(e){
    e.preventDefault();const b=e.submitter;TF.setLoading(b,true,'Refunding…');
    try{
      const type=TF.$('refundType').value;
      if(type==='partial'&&TF.$('refundReturnStock').checked)throw new Error('Stock return is only available for a full refund.');
      const {error}=await TF.state.supa.rpc('refund_order_v5',{p_order_id:activeDetailOrder.id,p_refund_date:TF.$('refundDate').value,p_amount:TF.num(TF.$('refundAmount').value),p_cash_account_id:TF.$('refundAccount').value,p_refund_type:type,p_return_all_stock:TF.$('refundReturnStock').checked,p_reason:TF.$('refundReason').value.trim()});
      if(error)throw error;TF.$('refundOrderDialog').close();TF.$('orderDetailDialog').close();TF.toast('Refund recorded');await loadOrders();
    }catch(err){TF.fail(err,'Refund not recorded');}finally{TF.setLoading(b,false);}
  }
  ...
  async function orderAction(e){
    ...
    try{
      ...
      if(b.dataset.action==='ship'){const tracking=prompt('Tracking number / release reference:',o.tracking_number||'')||'';let courier=0;if(o.fulfillment_method==='jnt'){const entered=prompt('Actual J&T courier cost (leave blank if still unknown):',o.courier_cost_finalized?String(o.actual_courier_cost||0):'');courier=entered===null||entered.trim()===''?null:Number(entered);}const {error}=await TF.state.supa.rpc('update_order_fulfillment',{p_order_id:id,p_status:'shipped',p_tracking_number:tracking,p_actual_courier_cost:courier});if(error)throw error;TF.toast('Order marked shipped/released');}
      if(b.dataset.action==='deliver'){const {error}=await TF.state.supa.rpc('update_order_fulfillment',{p_order_id:id,p_status:'delivered',p_tracking_number:o.tracking_number,p_actual_courier_cost:o.courier_cost_finalized?o.actual_courier_cost:null});if(error)throw error;TF.toast('Order delivered');}
  ...
  TF.ready.then(()=>{
    ...
    TF.$('refundOrderBtn').addEventListener('click',openRefundDialog);TF.$('refundOrderForm').addEventListener('submit',submitRefund);TF.$('closeRefundOrder').addEventListener('click',()=>TF.$('refundOrderDialog').close());TF.$('cancelRefundOrder').addEventListener('click',()=>TF.$('refundOrderDialog').close());TF.$('refundType').addEventListener('change',()=>{if(TF.$('refundType').value==='partial')TF.$('refundReturnStock').checked=false;});
    const updateDetailShippingPreview=()=>{const j=TF.$('detailFulfillmentInput').value==='jnt';TF.$('detailShippingInput').disabled=!j;TF.$('detailCourierInput').disabled=!j;if(!j){TF.$('detailShippingInput').value='0';TF.$('detailCourierInput').value='';}const cr=TF.$('detailCourierInput').value.trim();TF.$('detailShippingProfitPreview').textContent=cr===''?'Pending':TF.money(TF.num(TF.$('detailShippingInput').value)-TF.num(cr));};
