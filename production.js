(() => {
  ...
  const S={
    batches:[],rawBatches:[],cutting:[],cuttingPayments:[],payables:[],payablePayments:[],
    partners:[],people:[],assignments:[],partnerStatus:[],pulloutHistory:[],transferHistory:[],
    subconHistory:[],disbursements:[],suppliers:[],supplierItems:[],supplierPayments:[],supplierReceiptItems:[],fabric:[]
  };
  ...
  function initFields(){
    ...
    ['cutDate','wipBatchDate','assignDate','pulloutDate','transferDate','fabricDate','subconDate','payPayableDate','advanceDate','supplierPayDate','supplierReceiveDate'].forEach(id=>{if(TF.$(id))TF.$(id).value=today;});
    TF.$('cutCategory').innerHTML=TF.categoryOptions();TF.$('subconCategory').innerHTML=TF.categoryOptions();
    ['cutAccount','pulloutAccount','transferAccount','subconAccount','payPayableAccount','advanceAccount','supplierPayAccount','supplierFinalPaymentAccount','fabricAccount'].forEach(id=>TF.$(id).innerHTML=accountOptions(id==='advanceAccount'||id==='supplierFinalPaymentAccount'));
    ...
    updateTransferPaymentMode();
  }

  async function load(){
    try{
      const [batches,rawBatches,cutting,cuttingPayments,payables,payablePayments,partners,people,assignments,partnerStatus,pulloutHistory,transferHistory,subconHistory,disbursements,suppliers,supplierItems,supplierPayments,supplierReceiptItems,fabric]=await Promise.all([
        ...
        query('v_production_partner_batch_status_v13_2','*',q=>q.order('batch_number').order('partner_name')),
        query('v_production_pullout_history_v13_2','*',q=>q.order('pullout_date',{ascending:false}).order('created_at',{ascending:false})),
        query('v_production_assignment_transfers_v13_2','*',q=>q.order('transfer_date',{ascending:false}).order('created_at',{ascending:false})),
        query('v_subcon_receipt_history_v13_2','*',q=>q.order('receipt_date',{ascending:false}).order('created_at',{ascending:false})),
        query('v_payment_disbursements_v13_2','*',q=>q.order('payment_date',{ascending:false}).order('created_at',{ascending:false})),
      ...
      Object.assign(S,{batches,rawBatches,cutting,cuttingPayments,payables,payablePayments,partners,people,assignments,partnerStatus,pulloutHistory,transferHistory,subconHistory,disbursements,suppliers,supplierItems,supplierPayments,supplierReceiptItems,fabric});
      fillDynamicOptions();renderAll();
    }catch(e){TF.fail(e,'Production data failed');}
  }

  function fillDynamicOptions(){
    TF.$('cutFabricPurchase').innerHTML='<option value="">No linked fabric purchase</option>'+S.fabric.filter(f=>TF.num(f.total_cost)-TF.num(f.allocated_cost)>.009).map(f=>{const available=Math.max(TF.num(f.total_cost)-TF.num(f.allocated_cost),0);return `<option value="${f.id}" data-available="${available.toFixed(2)}">${date(f.purchase_date)} — ${TF.esc(f.supplier||'Fabric')} — ${fmt(available)} available of ${fmt(f.total_cost)} landed</option>`;}).join('');
    ...
    TF.$('assignBatch').innerHTML=batchOpt;TF.$('pulloutBatch').innerHTML=batchOpt;TF.$('transferBatch').innerHTML=batchOpt;
    TF.$('assignPartner').innerHTML=partnerOptions(activeOwnPartners());
    updatePulloutPartners();updateTransferPartners();updateSubconPartners();
  }

  function renderAll(){renderKpis();renderBatches();renderCutting();renderCuttingPayments();renderPayables();renderPartners();renderPartnerStatus();renderPulloutHistory();renderTransferHistory();renderSubconHistory();renderDisbursements();renderSuppliers();renderFabric();updateSummaries();}
  function renderKpis(){
  ...
  function renderCuttingPayments(){
    ...
    S.cuttingPayments.forEach(x=>{const job=byId(S.cutting,x.cutting_job_id),key=x.cash_transaction_id||x.id;seen.add(key);rows.push({date:x.payment_date,job:job?.job_code||'Cutting job',cutter:job?.cutter_name||'Unassigned',amount:x.amount,account:x.cash_accounts?.name||'',reference:x.reference_number||x.notes||'',key});});
  ...
  function renderPartners(){
    TF.$('partnersTable').innerHTML=`<table><thead><tr><th>Name</th><th>Type</th><th>Default rate</th><th>Assigned work</th><th>Available advance</th><th>Needs details</th><th>Open payable</th><th></th></tr></thead><tbody>${S.partners.filter(p=>p.role_type!=='cutter').map(p=>`<tr><td><strong>${TF.esc(p.name)}</strong><br><small>${TF.esc(p.category_name||'All categories')}</small></td><td>${TF.esc(p.role_type.replaceAll('_',' '))}</td><td>${fmt(p.default_rate)}/pc</td><td>${p.active_batches} batch(es) · ${p.assigned_quantity||0} pcs</td><td>${fmt(p.confirmed_advance_balance)}</td><td>${fmt(p.declared_unconfirmed_advance)}</td><td>${fmt(p.open_payables)}</td><td>${TF.num(p.declared_unconfirmed_advance)>0?`<button class="btn primary small" data-confirm-advance="${p.id}">Enter payment details</button>`:''}</td></tr>`).join('')}</tbody></table>`;
  }
  function renderPartnerStatus(){
    TF.$('partnerStatusTable').innerHTML=`<table><thead><tr><th>Batch</th><th>Partner</th><th>Assigned</th><th>Completed</th><th>Remaining</th><th>Labor</th><th>Advance</th><th>Cash</th><th>Open</th></tr></thead><tbody>${S.partnerStatus.map(x=>`<tr><td><strong>${TF.esc(x.batch_number)}</strong><br><small>${TF.esc(x.category_name)}</small></td><td>${TF.esc(x.partner_name)}</td><td>${x.assigned_quantity}</td><td>${x.completed_quantity}</td><td><strong>${x.remaining_quantity}</strong></td><td>${fmt(x.sewer_cost)}</td><td>${fmt(x.advance_applied)}</td><td>${fmt(x.cash_paid)}</td><td><strong>${fmt(x.open_payable)}</strong></td></tr>`).join('')||'<tr><td colspan="9" class="empty">No active partner assignments.</td></tr>'}</tbody></table>`;
  }
  function renderPulloutHistory(){
    TF.$('pulloutHistoryTable').innerHTML=`<table><thead><tr><th>Date</th><th>Batch / partner</th><th>Output</th><th>Labor</th><th>Advance</th><th>Cash</th><th>Payable</th><th>Pickup</th><th>Inventory value</th></tr></thead><tbody>${S.pulloutHistory.map(x=>`<tr><td>${date(x.pullout_date)}</td><td><strong>${TF.esc(x.batch_number)}</strong><br><small>${TF.esc(x.partner_name||'Unlinked partner')}</small></td><td>${x.completed_quantity} pcs</td><td>${fmt(x.sewer_pay)}</td><td>${fmt(x.advance_applied)}</td><td>${fmt(x.cash_paid)}${x.cash_account_name?`<br><small>${TF.esc(x.cash_account_name)}</small>`:''}</td><td><strong>${fmt(x.payable_remaining)}</strong></td><td>${fmt(x.pickup_delivery_cost)}</td><td>${fmt(x.receipt_cost)}<br><small>${fmt(x.provisional_unit_cost)}/pc</small></td></tr>`).join('')||'<tr><td colspan="9" class="empty">No pullouts recorded.</td></tr>'}</tbody></table>`;
  }
  function renderTransferHistory(){
    TF.$('transferHistoryTable').innerHTML=`<table><thead><tr><th>Date</th><th>Batch</th><th>From</th><th>To</th><th>Pieces</th><th>Delivery</th><th>Payment</th><th>Notes</th></tr></thead><tbody>${S.transferHistory.map(x=>`<tr><td>${date(x.transfer_date)}</td><td><strong>${TF.esc(x.batch_number)}</strong></td><td>${TF.esc(x.from_partner_name)}</td><td>${TF.esc(x.to_partner_name)}</td><td>${x.quantity}</td><td>${fmt(x.delivery_fee)}</td><td>${x.payment_mode==='reimbursement_payable'?`Reimburse ${TF.esc(x.reimbursement_party_name||'')}`:x.payment_mode==='paid_before_system'?'Paid before system':TF.esc(x.cash_account_name||'—')}</td><td>${TF.esc(x.notes||'')}</td></tr>`).join('')||'<tr><td colspan="8" class="empty">No partner transfers.</td></tr>'}</tbody></table>`;
  }
  function renderSubconHistory(){
    TF.$('subconHistoryTable').innerHTML=`<table><thead><tr><th>Date</th><th>Batch / partner</th><th>Category</th><th>Qty</th><th>Gross</th><th>Advance</th><th>Cash paid</th><th>Remaining</th><th>Status</th></tr></thead><tbody>${S.subconHistory.map(x=>`<tr><td>${date(x.receipt_date)}</td><td><strong>${TF.esc(x.batch_number)}</strong><br><small>${TF.esc(x.partner_name||'')}</small></td><td>${TF.esc(x.category_name)}</td><td>${x.good_finished_quantity}</td><td>${fmt(x.gross_cost??x.total_batch_cost)}</td><td>${fmt(x.advance_applied)}</td><td>${fmt(x.cash_paid)}</td><td><strong>${fmt(x.payable_remaining)}</strong></td><td>${TF.statusPill(x.payable_status)}</td></tr>`).join('')||'<tr><td colspan="9" class="empty">No full-subcon receipts.</td></tr>'}</tbody></table>`;
  }
  function renderDisbursements(){
    TF.$('disbursementHistoryTable').innerHTML=`<table><thead><tr><th>Date</th><th>Payment</th><th>Party</th><th>Amount</th><th>Paid from</th><th>Allocated to</th><th>Reference / notes</th></tr></thead><tbody>${S.disbursements.map(x=>`<tr><td>${date(x.payment_date)}</td><td><strong>${TF.esc(x.disbursement_code)}</strong></td><td>${TF.esc(x.party_name)}</td><td><strong>${fmt(x.total_amount)}</strong></td><td>${TF.esc(x.cash_account_name||'—')}</td><td><small>${safeLines(x.allocation_summary)}</small></td><td>${TF.esc(x.reference_number||x.notes||'—')}</td></tr>`).join('')||'<tr><td colspan="7" class="empty">No combined payable payments.</td></tr>'}</tbody></table>`;
  }
  ...
  function renderFabric(){
    TF.$('fabricTable').innerHTML=`<table><thead><tr><th>Date</th><th>Supplier</th><th>Description</th><th>Fabric purchase</th><th>Inbound</th><th>Total landed</th><th>Locked / allocated</th><th>Available</th></tr></thead><tbody>${S.fabric.map(f=>{const inbound=TF.num(f.inbound_delivery_cost),landed=TF.num(f.total_cost),base=Math.max(landed-inbound,0),available=Math.max(landed-TF.num(f.allocated_cost),0);return `<tr><td>${date(f.purchase_date)}</td><td>${TF.esc(f.supplier||'—')}</td><td>${TF.esc(f.description||'')}</td><td>${fmt(base)}</td><td>${fmt(inbound)}</td><td><strong>${fmt(landed)}</strong></td><td>${fmt(f.allocated_cost)}</td><td><strong>${fmt(available)}</strong></td></tr>`;}).join('')||'<tr><td colspan="8" class="empty">No fabric purchases.</td></tr>'}</tbody></table>`;
  }

  function updatePulloutPartners(){
    const batch=byId(S.rawBatches,TF.$('pulloutBatch').value),asg=S.partnerStatus.filter(a=>a.production_batch_id===batch?.id&&a.remaining_quantity>0);
    TF.$('pulloutPartner').innerHTML='<option value="">Select assigned partner</option>'+asg.map(a=>{const p=byId(S.people,a.partner_id);return `<option value="${a.partner_id}" data-rate="${p?.default_rate||9}">${TF.esc(a.partner_name)} — ${a.remaining_quantity} remaining of ${a.assigned_quantity}</option>`;}).join('');
  }
  function updateTransferPartners(){
    const batchId=TF.$('transferBatch').value,rows=S.partnerStatus.filter(x=>x.production_batch_id===batchId&&x.remaining_quantity>0);
    const selectedFrom=TF.$('transferFrom').value;
    TF.$('transferFrom').innerHTML='<option value="">Select source partner</option>'+rows.map(x=>`<option value="${x.partner_id}" data-remaining="${x.remaining_quantity}">${TF.esc(x.partner_name)} — ${x.remaining_quantity} unfinished</option>`).join('');
    if(rows.some(x=>x.partner_id===selectedFrom))TF.$('transferFrom').value=selectedFrom;
    const batch=byId(S.rawBatches,batchId),from=TF.$('transferFrom').value;
    TF.$('transferTo').innerHTML=partnerOptions(activeOwnPartners(batch?.category_id).filter(x=>x.id!==from),'Select destination partner');
    updateSummaries();
  }
  ...
  function updateTransferPaymentMode(){
    const fee=TF.num(TF.$('transferFee')?.value),mode=TF.$('transferPaymentMode')?.value;
    if(!TF.$('transferAccountWrap'))return;
    const needsAccount=fee>0&&mode==='company_cash',needsParty=fee>0&&mode==='reimbursement_payable';
    TF.$('transferAccountWrap').classList.toggle('field-disabled',!needsAccount);TF.$('transferAccount').disabled=!needsAccount;
    TF.$('transferReimbursementWrap').classList.toggle('hidden',!needsParty);TF.$('transferReimbursementParty').required=needsParty;
  }
  function updateSummaries(){
    ...
    const job=byId(S.cutting,TF.$('wipJob').value);TF.$('wipSummary').textContent=job?`${job.wip_quantity} pcs available · current WIP value ${fmt(job.wip_total_value)} · value transfers automatically`:'Choose a cutting job to see available WIP.';if(job)TF.$('wipQty').max=job.wip_quantity;
    ...
    const sq=TF.num(TF.$('subconQty').value),sp=TF.num(TF.$('subconPrice').value),sh=TF.num(TF.$('subconHandling').value),sd=TF.num(TF.$('subconPaid').value),st=sq*sp+sh,subPartner=byId(S.partners,TF.$('subconPartner').value),subAdvance=Math.min(Math.max(TF.num(subPartner?.confirmed_advance_balance),0),st);
    TF.$('subconSummary').textContent=sq&&sp?`Gross cost ${fmt(st)} · credit applied ${fmt(subAdvance)} · cash now ${fmt(sd)} · payable ${fmt(Math.max(st-subAdvance-sd,0))}`:'Enter quantity and price to see the total cost.';
    const fromOpt=TF.$('transferFrom').selectedOptions[0],remaining=TF.num(fromOpt?.dataset.remaining),transferQty=TF.num(TF.$('transferQty').value),fee=TF.num(TF.$('transferFee').value);
    TF.$('transferSummary').textContent=fromOpt?.value?`${remaining} unfinished pieces currently available with ${fromOpt.textContent.split(' — ')[0]}. After transfer: ${Math.max(remaining-transferQty,0)} pieces. Delivery: ${fmt(fee)}.`:'Choose a batch and source partner to see remaining assigned pieces.';
    if(remaining)TF.$('transferQty').max=remaining;
  ...
  function openBatch(id){
    const b=byId(S.batches,id);if(!b)return;const ps=S.pulloutHistory.filter(x=>x.production_batch_id===id).sort((a,z)=>String(a.pullout_date).localeCompare(String(z.pullout_date))),asg=S.partnerStatus.filter(x=>x.production_batch_id===id),transfers=S.transferHistory.filter(x=>x.production_batch_id===id);
    TF.$('batchDialogTitle').textContent=b.batch_number;TF.$('batchDialogMeta').textContent=`${b.category_name} · ${b.partner_name}`;
    TF.$('batchDialogBody').innerHTML=`<div class="detail-grid ops-detail"><div><span>Fabric</span><strong>${fmt(b.fabric_cost_used)}</strong></div><div><span>Garters</span><strong>${fmt(b.garter_cost)}</strong></div><div><span>Cutting</span><strong>${fmt(b.cutting_cost)}</strong></div><div><span>Inbound deliveries</span><strong>${fmt(TF.num(b.inbound_delivery_cost)+TF.num(b.pullout_delivery_cost))}</strong></div><div><span>Sewing completed</span><strong>${fmt(b.completed_sewing_cost)}</strong></div><div><span>Expected remaining sewing</span><strong>${fmt(b.expected_remaining_sewing)}</strong></div><div><span>Estimated final cost</span><strong>${fmt(b.estimated_final_cost)}</strong></div><div><span>Estimated unit cost</span><strong>${fmt(b.estimated_unit_cost)}</strong></div><div><span>Finished inventory value received</span><strong>${fmt(b.received_inventory_value)}</strong></div><div><span>Open batch payables</span><strong>${fmt(b.open_payables)}</strong></div></div><div class="notice info ops-formula"><strong>Formula:</strong> fabric + garters + cutting + inbound delivery + completed sewing + expected remaining sewing = estimated final batch cost.</div><h4>Partner assignments</h4><div class="table-wrap"><table><thead><tr><th>Partner</th><th>Assigned</th><th>Completed</th><th>Remaining</th><th>Open balance</th></tr></thead><tbody>${asg.map(a=>`<tr><td>${TF.esc(a.partner_name)}</td><td>${a.assigned_quantity}</td><td>${a.completed_quantity}</td><td><strong>${a.remaining_quantity}</strong></td><td>${fmt(a.open_payable)}</td></tr>`).join('')||'<tr><td colspan="5">No active assignment.</td></tr>'}</tbody></table></div><h4>Assignment transfers</h4><div class="table-wrap"><table><thead><tr><th>Date</th><th>From</th><th>To</th><th>Qty</th><th>Delivery</th></tr></thead><tbody>${transfers.map(x=>`<tr><td>${date(x.transfer_date)}</td><td>${TF.esc(x.from_partner_name)}</td><td>${TF.esc(x.to_partner_name)}</td><td>${x.quantity}</td><td>${fmt(x.delivery_fee)}</td></tr>`).join('')||'<tr><td colspan="5">No transfers.</td></tr>'}</tbody></table></div><h4>Pullouts</h4><div class="table-wrap"><table><thead><tr><th>Date</th><th>Partner</th><th>Good</th><th>Labor</th><th>Advance</th><th>Cash</th><th>Payable</th><th>Pickup</th><th>Receipt cost</th></tr></thead><tbody>${ps.map(p=>`<tr><td>${date(p.pullout_date)}</td><td>${TF.esc(p.partner_name||'')}</td><td>${p.completed_quantity}</td><td>${fmt(p.sewer_pay)}</td><td>${fmt(p.advance_applied)}</td><td>${fmt(p.cash_paid)}</td><td>${fmt(p.payable_remaining)}</td><td>${fmt(p.pickup_delivery_cost)}</td><td>${fmt(p.receipt_cost)}</td></tr>`).join('')||'<tr><td colspan="9">No pullouts.</td></tr>'}</tbody></table></div>`;
  ...
  function openAdvance(partnerId){const p=byId(S.partners,partnerId),d=S.people.find(x=>x.id===partnerId);if(!p||!d)return;TF.$('advanceId').value=partnerId;TF.$('advanceMeta').textContent=`${p.name} · ${fmt(p.declared_unconfirmed_advance)} needs payment date and account`;TF.$('advanceDialog').showModal();}
  ...
  function openSupplier(id){
    const o=byId(S.suppliers,id);if(!o)return;const items=S.supplierItems.filter(x=>x.purchase_order_id===id),pays=S.supplierPayments.filter(x=>x.purchase_order_id===id);
    TF.$('supplierDialogTitle').textContent=o.supplier_name;TF.$('supplierDialogMeta').textContent=`${o.order_code} · balance ${fmt(o.remaining_balance)}`;TF.$('supplierPayOrderId').value=id;TF.$('supplierReceiveOrderId').value=id;TF.$('supplierPayAmount').value=TF.num(o.remaining_balance).toFixed(2);TF.$('supplierPayReference').value='';TF.$('supplierPayNotes').value='';TF.$('supplierFinalPayment').value='0';TF.$('supplierFinalPaymentAccount').value='';TF.$('supplierReceiveReference').value='';TF.$('supplierReceiveNotes').value='';TF.$('supplierReceiveDate').value=TF.today();
  ...
  async function savePullout(e){e.preventDefault();const b=e.submitter,paid=TF.num(TF.$('pulloutPaid').value);await rpc('add_production_pullout_v13',{p_batch_id:TF.$('pulloutBatch').value,p_partner_id:TF.$('pulloutPartner').value,p_pullout_date:TF.$('pulloutDate').value,p_completed_quantity:TF.num(TF.$('pulloutQty').value),p_sewer_rate:TF.num(TF.$('pulloutSewerRate').value),p_amount_paid:paid,p_cash_account_id:paid>0?TF.$('pulloutAccount').value:null,p_final_pullout:TF.$('pulloutFinal').checked,p_rejected_quantity:TF.num(TF.$('pulloutRejects').value),p_notes:TF.$('pulloutNotes').value.trim()},'Pullout added to inventory',b);e.target.reset();initFields();}
  async function saveTransfer(e){
    e.preventDefault();const b=e.submitter,fee=TF.num(TF.$('transferFee').value),mode=TF.$('transferPaymentMode').value;
    await rpc('transfer_production_assignment_v13_2',{p_batch_id:TF.$('transferBatch').value,p_from_partner_id:TF.$('transferFrom').value,p_to_partner_id:TF.$('transferTo').value,p_transfer_date:TF.$('transferDate').value,p_quantity:TF.num(TF.$('transferQty').value),p_delivery_fee:fee,p_payment_mode:mode,p_cash_account_id:fee>0&&mode==='company_cash'?TF.$('transferAccount').value:null,p_reimbursement_party_name:fee>0&&mode==='reimbursement_payable'?TF.$('transferReimbursementParty').value.trim():null,p_reference_number:TF.$('transferReference').value.trim(),p_notes:TF.$('transferNotes').value.trim()},'Partner assignment transferred',b);e.target.reset();TF.$('transferDate').value=TF.today();TF.$('transferFee').value='0';TF.$('transferPaymentMode').value='company_cash';updateTransferPaymentMode();
  }
  ...
  async function saveSubcon(e){
    e.preventDefault();const b=e.submitter,paid=TF.num(TF.$('subconPaid').value);
    await rpc('receive_subcon_batch_v13_2',{p_batch_number:TF.$('subconBatchNumber').value.trim(),p_category_code:TF.$('subconCategory').value,p_received_date:TF.$('subconDate').value,p_partner_id:TF.$('subconPartner').value,p_quantity:TF.num(TF.$('subconQty').value),p_price_per_piece:TF.num(TF.$('subconPrice').value),p_handling_cost:TF.num(TF.$('subconHandling').value),p_amount_paid:paid,p_cash_account_id:paid>0?TF.$('subconAccount').value:null,p_reference_number:TF.$('subconReference').value.trim(),p_notes:TF.$('subconNotes').value.trim()},'Subcon inventory received',b);e.target.reset();initFields();
  }
  ...
  async function receiveSupplier(e){e.preventDefault();const b=e.submitter,items=[...TF.$('supplierReceiveItems').querySelectorAll('[data-receive-item]')].map(x=>({purchase_order_item_id:x.dataset.receiveItem,quantity_received:TF.num(x.value)})).filter(x=>x.quantity_received>0);await rpc('receive_supplier_order_v13',{p_purchase_order_id:TF.$('supplierReceiveOrderId').value,p_receipt_date:TF.$('supplierReceiveDate').value,p_items:items,p_delivery_cost:0,p_delivery_cash_account_id:null,p_final_payment:TF.num(TF.$('supplierFinalPayment').value),p_payment_cash_account_id:TF.num(TF.$('supplierFinalPayment').value)>0?TF.$('supplierFinalPaymentAccount').value:null,p_reference_number:TF.$('supplierReceiveReference').value.trim(),p_notes:TF.$('supplierReceiveNotes').value.trim()},'Supplier stock received. Record any Lalamove fee under Inbound Deliveries.',b);TF.$('supplierDialog').close();}
  ...
  function bind(){
    ...
    TF.$('cuttingForm').addEventListener('submit',saveCutting);TF.$('wipBatchForm').addEventListener('submit',createWipBatch);TF.$('partnerAssignmentForm').addEventListener('submit',assignPartner);TF.$('pulloutForm').addEventListener('submit',savePullout);TF.$('partnerTransferForm').addEventListener('submit',saveTransfer);TF.$('payPayableForm').addEventListener('submit',savePay);TF.$('advanceForm').addEventListener('submit',confirmAdvance);TF.$('fabricPurchaseForm').addEventListener('submit',saveFabric);TF.$('subconForm').addEventListener('submit',saveSubcon);TF.$('supplierPaymentForm').addEventListener('submit',paySupplier);TF.$('supplierReceiveForm').addEventListener('submit',receiveSupplier);
    TF.$('pulloutBatch').addEventListener('change',()=>{updatePulloutPartners();updateSummaries();});TF.$('pulloutPartner').addEventListener('change',e=>{const o=e.target.selectedOptions[0];if(o?.dataset.rate)TF.$('pulloutSewerRate').value=o.dataset.rate;updateSummaries();});
    TF.$('transferBatch').addEventListener('change',updateTransferPartners);TF.$('transferFrom').addEventListener('change',updateTransferPartners);TF.$('transferPaymentMode').addEventListener('change',updateTransferPaymentMode);
    TF.$('subconCategory').addEventListener('change',updateSubconPartners);TF.$('subconPartner').addEventListener('change',e=>{const p=byId(S.people,e.target.value);if(p)TF.$('subconPrice').value=p.default_rate;updateSummaries();});TF.$('cutFabricPurchase').addEventListener('change',e=>{const o=e.target.selectedOptions[0];if(o?.dataset.available)TF.$('cutFabricCost').value=o.dataset.available;});
    ['cutUsableQty','cutRate','cutPaid','wipJob','pulloutQty','pulloutSewerRate','pulloutPaid','transferQty','transferFee','subconQty','subconPrice','subconHandling','subconPaid'].forEach(id=>TF.$(id).addEventListener('input',updateSummaries));
